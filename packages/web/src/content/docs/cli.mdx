...
For OpenAI OAuth, each successful login adds an account to the `openai_oauth_accounts` list keyed by account user identity (`chatgpt_account_user_id` when available). OpenCode automatically switches between these accounts in-memory when the current account runs out of quota.
